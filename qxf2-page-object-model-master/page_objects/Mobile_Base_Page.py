...
class Mobile_Base_Page(Borg,unittest.TestCase):
    ...
    def __init__(self,driver = None):
        ...
        if self.is_first_time():
            ...
            self.reset()
            print("初始化设置")
        #判断driver是否为空，为空使用driver工厂模式获取driver
    ...
    def start_activity(self, app_package, app_activity,appWaitDuration = 20):
        
        self.write("change app begin:"+app_package,level="info")
        try:
            self.driver.start_activity(app_package,app_activity,appWaitDuration=appWaitDuration)
        except Exception as e:
            self.write(str(e),level = "error")
            self.write("the current activity "+str(self.driver.current_activity))
            self.wait(10)
        self.write("change app,the current activity ")
        self.write(self.driver.current_activity)
    ...
    def save_screenshot(self,screenshot_name):
        ...
        self.write("save screenshot "+screenshot_name)
        return self.screenshot_dir + os.sep+ screenshot_name+'.png'
